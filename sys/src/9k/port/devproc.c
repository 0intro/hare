...
/* print functions for trace system call. These are portable and hence here instead of
 * in e.g. bgp/trap.c. 
 */
static void
...
/* next two functions are called with the 'sp' array as found in trap.c. 
 * on almost all architectures we have seen it is just a pointer to the user stack. 
 * The convention is that the sp array holds the array of arguments to the system call, 
 * starting at offset 1. Technically sp[0] could be the syscall number but most 
 * architectures put that in a register (e.g. r0 or ax) for performance reasons. 
 */
void
syscallprint(int syscallno, uintptr *sp)
{
	vlong offset;
	Fmt fmt;
	int len;
	...
	if(syscallno > nsyscall)
		...
		fmtprint(&fmt, "%s %#ulx ", systab[syscallno].n, sp[0]);
	...
	case EXEC: 
		...
		evenaddr(sp[2]);
		argp = (char**)sp[2];
		validaddr(argp, BY2SE, 0);
		while(*argp){
			a = *argp++;
			if(((ulong)argp&(BY2PG-1)) < BY2SE)
				validaddr(argp, BY2SE, 0);
			if(a == 0)
				break;
			fmtprint(&fmt, " ");
...
void
retprint(int syscallno, uintptr *sp, Ar0 *ar0, uvlong start, uvlong stop)
{
	int errstrlen, len;
	vlong offset;
	char *errstr;
	...
	case AWAIT:
		if(retval> 0){
			fmtuserstring(&fmt, sp[1], " ");
			fmtprint(&fmt, "%ld", sp[2]);
		} else {
			fmtprint(&fmt, "%#ulx/\"\" %ld", sp[1], sp[2]);
		}
		break;
	case _ERRSTR:
		errstrlen = 64;
	...
	case PREAD:
		if(retval == -1)
			fmtprint(&fmt, "/\"\" %ld 0x%ullx", sp[3], *(vlong *)&sp[4]);
		else {
			if (retval > 64)
				len = 64;
			else
