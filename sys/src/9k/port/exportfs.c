/* in-kernel exportfs, from inferno */
#include	"u.h"
#include	"../port/lib.h"
#include	"mem.h"
#include	"dat.h"
...
exreadmsg(Chan *c, void *a, uint n)
{
	...
	if(len <= BIT32SZ || len > n){
		werrstr("bad length in Styx message header");
		return -1;
	}
	len -= BIT32SZ;
...
exportproc(void *a)
{
	...
	for(;;){
		...
		if(q == nil){
			werrstr("out of memory: read request");
			n = -1;
			break;
		}
		memset(q, 0, sizeof(*q));
		...
		if(convM2S(q->buf, n, &q->in) != n){
			werrstr("bad T-message");
			n = -1;
			break;
		}
		type = q->in.type;
		if(type < Tversion || type >= Tmax || type&1 || type == Terror){
			werrstr("invalid T-message type %d", type);
			n = -1;
			break;
		}
...
exmount(Chan *c, Mhead **mp, int doname)
{
	...
	nc = nil;
	if((c->flag & COPEN) == 0 && findmount(&nc, mp, c->dev->dc, c->devno, c->qid)){
		if(waserror()){
			cclose(nc);
			nexterror();
		}
...
//	r->count = devtab[c->type]->write(c, t->data, t->count, t->offset);
	r->count = c->dev->read(c, t->data, t->count, t->offset);
	poperror();
	Exputfid(fs, f);
	return nil;
}
...
uqidlook(Uqid **tab, Chan *c, vlong path)
{
	...
	for(hp = &tab[uqidhash(path)]; (q = *hp) != nil; hp = &q->next)
		if(q->dev == c->dev && q->devno == c->devno && q->oldpath == path)
			break;
	return hp;
}
...
long
sysexportfs(ulong *arg)
{
	struct {
		int	c;		/* fd of connection to client */
		char	*dir;		/* tree to export */
		int	flag;
	} *f;

	f = (void *)arg;
	validaddr(f->dir, 1, 0);
	return export(f->c, f->dir, f->flag); // f->flag & Sys_EXPASYNC);
}
