 ...
 */
int
iinit(Icache *ic, char *mc, int psize, char* name)
{
	Ibuf *b;
	Imap *m;
	ulong ino;
 ...
 */
int
iformat(Icache *ic, char *mc, ulong nino, char *name, int bsize, int psize)
{
	int nib;
	ulong bno, i2b, i;
	Bbuf *bb;
	 ...
	 */
	if(dformat(ic, mc, name, bsize, psize) < 0)
		return -1;

	fprint(2, "formatting inodes in memory\n");
