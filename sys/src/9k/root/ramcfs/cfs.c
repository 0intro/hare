...
main(int argc, char *argv[])
{
	...
	case 'm':
		...
		if (cachesize < 8 * 1024 * 1024 ||
			...
			sysfatal("implausible cache size %lud", cachesize);
		break;
	case 'n':
		noauth = 1;
		break;
...
void
cachesetup(int format, char *name, char *)
{
	int secsize;
	int inodes;
	int blocksize;
	...
	secsize = 512;		/* only really matters for disks */
	blocksize = 4*1024;
	inodes = 2*1024;

	memcache = malloc(cachesize);
	if(memcache == nil)
		error("can't allocate memory for cache: %r");

	/*
	 * Always format.  If we don't have a name, fall
	 * back to our old behavior of using "bootes"
	 */
	USED(format);
	name = (name == nil? "bootes": name);
	if(iformat(&ic, memcache, inodes, name, blocksize, secsize) < 0)
