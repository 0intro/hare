...
main()
{
	...
	close(control[0]);
	close(pipes[1]);
	buf = (char*)valloc(XFERSIZE + 4096);
	if (buf == NULL) {
		perror("no memory");
		return(1);
	}
	touch(buf, XFERSIZE + 4096);
	buf += 128;	/* destroy page alignment */
	BENCH(reader(control[1], pipes[0], XFER), MEDIUM);
	fprint(stderr, "Pipe bandwidth: ");
	mb(get_n() * XFER);
	postnote(PNPROC, pid, "eat flaming death yankee pigdog");
	return(0);
}
...
reader(int controlfd, int pipefd, size_t bytes)
{
	...
	if (n < 0) perror("reader::write");
	if (done < bytes) {
		fprintf(stderr, "reader: bytes=%lud, done=%d, todo=%lud\n", bytes, done, todo);
	}
}
