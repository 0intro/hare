...
ADC_VIEW_CNTL * NewAdcViewCntl(ADC_VIEW_PARS *adcpp, uint32 pnum){
   ...
   if ( adccntl->jpp == NULL){
      fprintf(adccntl->logf,
        "\n Not enough space to allocate %d byte for a job pool.", 
        (adccntl->nViewLimit+1)*sizeof(JOB_POOL));
      adccntl->retCode = ADC_MEMORY_ALLOCATION_FAILURE; 
      return(adccntl);
   }
   ...
   if ( adccntl->lpp == NULL){
      fprintf(adccntl->logf,
        "\n Not enough space to allocate %d byte for a layer reference array.", 
        (adcpp->nd+1)*sizeof(LAYER));
      adccntl->retCode = ADC_MEMORY_ALLOCATION_FAILURE;
      return(adccntl);
   }
