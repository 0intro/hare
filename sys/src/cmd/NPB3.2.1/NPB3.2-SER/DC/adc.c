...
void ShowFactorization(Factorization *nmbfct){
  ...
  for(i=0;i<nmbfct->dim;i++){
    if(nmbfct->mlt[i]==1){
      if(i==0) fprintf(stderr,"prime.");
      break;
    }
    if(i>0) fprintf(stderr,"*");
    if(nmbfct->exp[i]==1)
      fprintf(stderr,"%ld",nmbfct->mlt[i]);    
    else 
      fprintf(stderr,"%ld^%ld",nmbfct->mlt[i],
                               nmbfct->exp[i]);
  }
  fprintf(stderr,"\n");
}
...
/*
  fprintf(stderr,"ListFirstPrimes: listing primes less than %ld...\n",
                 mpr);
*/
  long int prnum=0;
  int composed=0;
...
/*  fprintf(stderr,"ListFirstPrimes: Done.\n"); */
  return prnum;
}
...
for(i=0;i<maxprmfctr;i++){
  if(expons[i]>0) fprintf(stderr,"*%ld^%ld",i,expons[i]);
}
fprintf(stderr,"\n");
*/
  for(i=0;i<=maxprmfctr;i++){
...
void GetFactorization(long int prnum,long int *prlist,
                            Factorization **fctlist){
/*fprintf(stderr,"GetFactorization: factorizing first %ld numbers.\n",
                prnum);*/
  long int i=0,j=0;
...
/*  fprintf(stderr,"GetFactorization: Done.\n"); */
}
...
int CalculateVeiwSizes(ADC_PAR *par){
  ...
  fclose(view);
  free(dcview);
  fprintf(stderr,"View sizes are written into %s\n",vszefname);
  free(vszefname);
  return 1;
}
...
int WriteADCPar(ADC_PAR *par,char* fname){
  ...
  fprintf(parfile,"attrNum=%ld\n",par->dim);
  fprintf(parfile,"measuresNum=%ld\n",par->mnum);
...
void ShowADCPar(ADC_PAR *par){
  fprintf(stderr,"********************* ADC paramters\n");
  fprintf(stderr," id		%ld\n",par->ndid);
  fprintf(stderr," attributes 	%ld\n",par->dim);
  fprintf(stderr," measures   	%ld\n",par->mnum);
  fprintf(stderr," tuples     	%lld\n",par->tuplenum);
  fprintf(stderr," class	\t%c\n",par->clss);
...
int GetNextTuple(int dcdim, int measnum,
  ...
  if(dcdim>maxdim){
    fprintf(stderr,"GetNextTuple: number of dcdim is too large:%ld",
                    dcdim);
    return 0;
  }
  if(measnum>measbound){
    fprintf(stderr,"GetNextTuple: number of mes is too large:%ld",
                    measnum);
    return 0;
  }
  locexp=adcexp;
  ...
  if(tuplenum==0){
    ...
    fprintf(stderr,"Prime \t Generator \t Seed\n");
    for(i=0;i<dcdim;i++){
...
/*  } */
  fclose(adc);
  fprintf(stderr,"Binary ADC file %s ",adcfname);
  fprintf(stderr,"have been generated.\n");
  free(attr);
  free(mes);
  free(adcfname);
