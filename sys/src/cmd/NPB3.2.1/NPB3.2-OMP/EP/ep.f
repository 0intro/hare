...
!$    write (*,1003) omp_get_max_threads()
 ...
 1003 format(' Number of available threads:        ', i14)
...
c         if (timers_enabled) call timer_start(3)
         call vranlc(2 * nk, t1, a, x)
c         if (timers_enabled) call timer_stop(3)

c        Compute Gaussian deviates by acceptance-rejection method and 
c        tally counts in concentric square annuli.  This loop is not 
c        vectorizable. 

c         if (timers_enabled) call timer_start(2)
...
c         if (timers_enabled) call timer_stop(2)
