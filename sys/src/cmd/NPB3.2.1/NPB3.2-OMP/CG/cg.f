...
!$    write( *,1003 ) omp_get_max_threads()
      write( *,* )
 1000 format(//,' NAS Parallel Benchmarks (NPB3.2-OMP)',
     >          ' - CG Benchmark', /)
...
c---------------------------------------------------------------------
     ...
     >                   r(*)
      ...
      rho = 0.0d0
!$omp parallel default(shared) private(j,sum) shared(rho,naa)
...
c---------------------------------------------------------------------
      do cgit = 1, cgitmax
         ...
         d = 0.d0
         rho = 0.d0
!$omp parallel default(shared) private(j,k,sum,alpha,beta)
!$omp& shared(d,rho0,rho)
...
c---------------------------------------------------------------------
      sum = 0.0d0
!$omp parallel default(shared) private(j,d)
!$omp& shared(sum)
!$omp do
      do j=1,lastrow-firstrow+1
         d = 0.d0
...
c---------------------------------------------------------------------
!$omp parallel default(shared) private(iouter,nzv,vc,ivc,ivelt,
!$omp&  work) shared(n,nn1)
      num_threads = 1
...
c---------------------------------------------------------------------
!$omp parallel default(shared) private(i,j,nza)
      do j = ilow+1, ihigh+1
         rowstr(j) = 0
      enddo
...
!$omp parallel default(shared) private(i,j,k,kk,jcol,nza,nzrow,
!$omp&   va,size,ratio,scale)
c---------------------------------------------------------------------
...
!$omp parallel default(shared) private(j,j1,j2,k,nza)
!$omp do
      do j = 1, nrows
         if (j .gt. 1) then
