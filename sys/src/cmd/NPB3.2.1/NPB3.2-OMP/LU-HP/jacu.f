...
c---------------------------------------------------------------------
      ...
      subroutine jacu(l, np, indxp, jndxp)
...
!$OMP& r43)
!$OMP&  SHARED(tz2,ty2,tx2,dz5,dy5,dx5,dz4,dy4,dx4,dz3,dy3,dx3,dz2,dy2,
...
c---------------------------------------------------------------------
     ...
     >         - dt * tx1 * ( - c34 * tmp2 * u(3,i+1,j,k) )
               a(3,2,n) =  dt * tx2 * ( u(3,i+1,j,k) * tmp1 )
               a(3,3,n) =  dt * tx2 * ( u(2,i+1,j,k) * tmp1 )
     >          - dt * tx1 * ( c34 * tmp1 )
     >          - dt * tx1 * dx3
               a(3,4,n) = 0.0d+00
               a(3,5,n) = 0.0d+00
     ...
     >          - dt * tx1 * ( - c34 * tmp2 * u(4,i+1,j,k) )
               a(4,2,n) = dt * tx2 * ( u(4,i+1,j,k) * tmp1 )
               a(4,3,n) = 0.0d+00
               a(4,4,n) = dt * tx2 * ( u(2,i+1,j,k) * tmp1 )
     >          - dt * tx1 * ( c34 * tmp1 )
     >          - dt * tx1 * dx4
               a(4,5,n) = 0.0d+00
...
c---------------------------------------------------------------------
     ...
     >           - dt * tz1 * ( - c34 * tmp2 * u(2,i,j,k+1) )
               c(2,2,n) = dt * tz2 * ( u(4,i,j,k+1) * tmp1 )
     >           - dt * tz1 * c34 * tmp1
     >           - dt * tz1 * dz2 
               c(2,3,n) = 0.0d+00
               c(2,4,n) = dt * tz2 * ( u(2,i,j,k+1) * tmp1 )
               c(2,5,n) = 0.0d+00
     ...
     >           - dt * tz1 * ( - c34 * tmp2 * u(3,i,j,k+1) )
               c(3,2,n) = 0.0d+00
               c(3,3,n) = dt * tz2 * ( u(4,i,j,k+1) * tmp1 )
     >           - dt * tz1 * ( c34 * tmp1 )
     >           - dt * tz1 * dz3
               c(3,4,n) = dt * tz2 * ( u(3,i,j,k+1) * tmp1 )
               c(3,5,n) = 0.0d+00
