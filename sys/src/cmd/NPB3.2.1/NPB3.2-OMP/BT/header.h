...
c
     ...
     >   forcing (5,     0:IMAX/2*2,  0:JMAX/2*2,  0:KMAX/2*2),
     >   u       (5,     0:IMAX/2*2,  0:JMAX/2*2,  0:KMAX/2*2),
...
c
      double precision fjac(5, 5, 0:problem_size/2*2),
     >                 njac(5, 5, 0:problem_size/2*2),
     >                 tmp1, tmp2, tmp3
      double precision lhs(5, 5, 3, 0:problem_size/2*2)
      common /work_lhs/ fjac, njac, tmp1, tmp2, tmp3, lhs
!$omp threadprivate (/work_lhs/)
c
c!      double precision  tmp_block(5,5), b_inverse(5,5), tmp_vec(5)
c!      common /work_solve/ tmp_block, b_inverse, tmp_vec
