#ifndef _ASYNC_TERMINATION_DETECTION_TREE_HPP_
#define _ASYNC_TERMINATION_DETECTION_TREE_HPP_
...
class async_termination_detection_tree {
  private:
  ...
  async_termination_detection_tree(MPI_Comm in_mpi_comm,
  ...
  bool test_for_termination(const size_type& in_sent, 
                            const size_type& in_received) {
    return test_for_termination_internal(in_sent, in_received);
  }

  private:
  bool test_for_termination_internal(const size_type& in_sent,
                                     const size_type& in_received) {
    switch(m_current_state) {
      case WAITING_INIT:
        return handle_waiting_init(in_sent, in_received);
      case WAITING_ISEND_CHILDREN:
        return handle_waiting_isend_children(in_sent, in_received);
      case WAITING_RECV_CHILDREN:
        return handle_waiting_recv_children(in_sent, in_received);
      case WAITING_ISEND_PARENT:
        return handle_waiting_isend_parent(in_sent, in_received);
    };
    return false;
  }

  bool handle_waiting_init(const size_type& in_sent,
                           const size_type& in_received) {
  ...
  bool handle_waiting_isend_children (const size_type& in_sent,
                                      const size_type& in_received) {
    //std::cout << m_mpi_rank << " " << __FUNCTION__ << std::endl;
  ...
  bool handle_waiting_recv_children (const size_type& in_sent,
                                     const size_type& in_received) {
    //std::cout << m_mpi_rank << " " << __FUNCTION__ << std::endl;
    if(is_leaf_rank()) {
      m_subtree_status_response.first += in_sent;
      m_subtree_status_response.second += in_received;
  ...
  bool handle_waiting_isend_parent (const size_type& in_sent,
                                    const size_type& in_received) {
    //std::cout << m_mpi_rank << " " << __FUNCTION__ << std::endl;
...
#endif //_ASYNC_TERMINATION_DETECTION_TREE_HPP_
