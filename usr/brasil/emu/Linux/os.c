...
mountyerself(char *mntpoint)
{
	...
	sprint(opts, "trans=fd,dfltuid=%d,dfltgid=%d,rfdno=%d,wfdno=%d,debug=0xffff", getuid(), getgid(), fd1[0], fd0[1]);
	
	if(fork() == 0) {
		ret = mount("nodev", mntpoint, "9p", 0, opts);
		if(ret < 0)
			perror("mount:");
		_exit(1);
	}
