...
procreadremote(void *a)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("PRR:No more data to read\n");
	freeb(content);
    ...
    if (vflag)
	print("PRR[%s]:No more data to read\n", rf->cfile->name->s);
    freeb(content);
    rf->state = 2;		/* thread dead */
    poperror();
    pexit("", 0);
...
proc_splice(void *param)
{
    ...
    if (vflag)
	print("proc_splice starting [%s]->[%s]\n", src->name->s,
	      dst->name->s);
    while (1) {
	...
	if (vflag)
	    print("proc_splice copying [%s]->[%s] %ld data \n",
    ...
    if (vflag)
	print("proc_splice closing [%s]->[%s] \n", src->name->s,
	      dst->name->s);

    cclose(src);
    cclose(dst);
    if (vflag)
	print("proc_splice complete \n");
...
remoteopen(Chan * c, int omode, char *fname, int filetype, int resno)
{
	...
	if (vflag)
	    print("opening file for first times[%d]\n",
	...
	if (vflag)
	    print("file is already open for [%d] times\n", tmpfileuse);
	wlock(&cv->rjob->l);
	cv->rjob->rcinuse[filetype] += 1;
	wunlock(&cv->rjob->l);
    }
...
closeconv(Conv * c)
{
    ...
    if (vflag)
	print("remote resources released\n");
    if (c->cmd != nil) {
	if (vflag)
	    print("freeing cmd\n");
	free(c->cmd);		/* FIXME: why this fails???? */
	if (vflag)
	    print("freeing cmd done\n");
    }
    c->cmd = nil;
    if (c->waitq != nil) {
	qfree(c->waitq);
...
remoteclose(Chan * c, int filetype, int rjcount)
{
	...
	if (vflag)
	    print("closing it as only one open in cmd[%d]\n", cc->x);
	wlock(&cc->rjob->l);
	...
	if (vflag)
	    print("not closing [%s],accessed by others [%d]\n", c->name->s,
		  tmpfilec);
	wlock(&cc->rjob->l);
...
cmdfdclose(Conv * c, int fd)
{
	...
	if (vflag)
	    print("calling close in cmdfdclose\n");
	close(c->fd[fd]);
	c->fd[fd] = -1;
    }
}
...
cmdclose(Chan * c)
{
	...
	if (tmpjc < 0) {
	     ...
	     */
	    //if (vflag)
	    print("returning as rjobcount is [%d]\n", tmpjc);
	    //return;
	}
	qlock(&cc->l);
...
readfromallasync(Chan * ch, void *a, long n, vlong offset)
{
    ...
    if (vflag)
	print("readfromallasync cname [%s]\n", ch->name->s);
	...
	if (ccount >= tmpjc) {
	    if (vflag)
		print("----readfromallasync fileclose [%ld]\n", ccount);
...
cmdread(Chan * ch, void *a, long n, vlong offset)
{
	...
	if (c->cmd != nil) {
	    if (vflag)
		print("came here 31 [%q]\n", c->dir);
	    cmds = c->cmd->f[1];
	    if (vflag)
		print("came here 32 [%q]\n", c->dir);
	    if (vflag)
		print("came here 33 [%q]\n", cmds);
	}
	if (vflag)
	    print("came here 4\n");
	...
	if (c->fd[fd] == -1) {
	    qunlock(&c->l);
	    if (vflag)
		print("file descriptor is closed\n");
	    return 0;
	}
	qunlock(&c->l);
	osenter();
...
validaterr(char *location, char *os, char *arch)
{
	...
	if (DMDIR & tmpDr->mode) {
		...
		if (vflag)
		    print("this is parent dir, ignoring\n");
		break;
	    }
	}
    }
    ...
    if (i == count) {
	if (vflag)
	    print("validaterr: [%s] not found, returning\n", localName);
	free(ans->path);
	free(ans);
	poperror();
	return nil;
    ...
    if (vflag)
	print("validaterr: now  searching for clone file\n");
...
findrr(int *validrc, char *os, char *arch)
{
	...
	*validrc = 0;
    ...
    if (vflag)
	print("remote dir has %ld entries\n", count);
    for (i = 0, tmpDr = dr; i < count; ++i, ++tmpDr) {
	snprint(location, sizeof(location), "%s/%s", path, tmpDr->name);

	if (vflag)
	    print("checking for remote location[%s]\n", location);
...
allocatesinglerr(void *info)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("#### error in sending reservation request to remote\n");
	/* report failure */
    ...
    if (ret != len) {
	if (vflag)
	    print("#### ERR : res write size(%d) returned [%d]\n",
		  len, ret);
	/* report failure */
	qwrite(r_file->async_queue, report, 2);
	pexit("", 0);
	    ...
	    Conv * c, int resno, int first, int others)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("Could not allocate needed rmt resources\n");
	...
	if (waserror()) {
	    if (vflag)
		print("could not bind remote and local resource\n");
	    error(ENOReservation);
	}
	...
	if (vflag)
	    print("clone returned %ld data [%s]\n",
	...
	if (ret != 2) {
	    ++flag;
	    if (vflag)
		print("qread on ctl buff failed\n");
	} else {
	    if (buf[1] != 1) {
		++flag;
		if (vflag)
		    print("resource allocation for [%d] failed\n", buf[0]);
	    } else {
		if (vflag)
		    print("resource allocation for [%d] successful\n",
			  buf[0]);
    ...
    if (flag > 0) {
	if (vflag)
	    print("resource allocation failed for [%d] times\n", flag);
...
groupres(Chan * ch, int resNo, char *os, char *arch)
{
	...
	/* no remote resources */
	if (vflag)
	    print("no remote resources,reserving [%d]locally\n", resNo);
	...
	if ((os != nil) && (os[0] != '*')) {
		...
		/* Wrong os requested */
		if (vflag)
		    print("Local OS differs requested OS\n");
	...
	if ((arch != nil) && (arch[0] != '*')) {
		...
		if (vflag)
		    print("Local platform differs requested one\n");
		error(ENoResourceMatch);
	    }
	}
...
p_send2one(void *param)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("###p_send2one[%d]: error in sending write\n",
		  rf->local_session_id);
	/* report failure */
    ...
    if (vflag)
	print("###p_send2one[%d]: about to write\n", rf->local_session_id);

    /* do actual write */
    ret = devtab[rf->cfile->type]->write(rf->cfile, a, n, offset);

    if (vflag)
	print("###p_send2one[%d]: write done\n", rf->local_session_id);
...
p_send2all(Chan * ch, void *a, long n, vlong offset)
{
    ...
    if (vflag)
	print("looping done anayling result\n");
    /* ensure that resources are properly allocated */
    ...
    if (vflag)
	print("analysis done, declaring\n");

    /* report failure */
    if (flag > 0) {
	if (vflag)
	    print("write failed for [%d] nodes\n", flag);
	error(ERemoteResFail);
    }
    if (vflag)
...
dolocalexecution(Conv * c, Cmdbuf * cb)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("so much for local execution\n");
	qunlock(&c->l);
	nexterror();
    }
    if (c->child != nil || c->cmd != nil) {
	if (vflag)
	    print("I like using things twice c->child %p c->cmd %p\n",
		  c->child, c->cmd);
	error(Einuse);
    }
    for (i = 0; i < nelem(c->fd); i++)
	if (c->fd[i] != -1) {
	    if (vflag)
		print("not the channel!\n");
	    error(Einuse);
	}
    if (cb->nf < 1) {
	if (vflag)
	    print("whoops I'm too small\n");
	error(Etoosmall);
    }
...
cmdwrite(Chan * ch, void *a, long n, vlong offset)
{
	...
	case CMres:
	    if (vflag)
		print("reserving resources %s \n", cb->f[1]);
	    resNo = atol(cb->f[1]);
	    if (vflag)
		print("reserving resources %s [%ld]\n", cb->f[1], resNo);

	    if (resNo < 0) {
		/* negative value for reservation is invalid */
		if (vflag)
		    print("Error: negative error value\n");
		error(Eperm);
	    }
	    if (tmpjc != -1) {
		if (vflag)
	...
	case CMexec:
	    ...
	    if (vflag)
		print("no of remote res are [%ld]\n", tmpjc);
	    if (tmpjc < -1) {
		if (vflag)
		    print("previous res command failed\n");
		error(ENOReservation);
	    }
	    /* request for execution of command */
	    if (tmpjc == -1) {
		if (vflag)
		    print("No reservation done\n");
		/* local execution request */
		...
		/* local execution request */
		if (vflag)
		    print("executing cmd locally\n");
		dolocalexecution(c, cb);
		poperror();
		return ret;
	    }
	    if (vflag)
		print("executing cammand remotely\n");
	    ret = p_send2all(ch, a, n, offset);
	    if (vflag)
		print("executing cammand - done\n");
	    break;
	...
	case CMkill:
		...
		/* local execution request */
		if (vflag)
		    print("executing kill locally\n");
		qlock(&c->l);
		if (waserror()) {
		    if (vflag)
			print("we aren't good killers\n");
		    qunlock(&c->l);
		    nexterror();
		}
		if (c->child == nil)
...
cmdproc(void *a)
{
	...
	if (vflag)
	    print
		("killin all the city of New Orleans, I'll have gone 5000 miles for the day is done\n");
	oscmdkill(t);
    }
    osenter();
...
myunionread(Chan * c, void *va, long n)
{
    ...
    if (vflag)
	print("inside myunionread for [%s]\n", c->name->s);
	...
	if (nr > 0) {
	    if (vflag)
		print("myunionread got something out[%s]\n",
		      c->umc->name->s);
	    /*
	    ...
	    if (a_n <= 0) {
		if (vflag)
		    print("myunionread Buffer full, breking");
		break;
	    }
	}
	/* end if : nr > 0 */
...
readfromchan(Chan * rc, void *va, long n, vlong off)
{
    ...
    if (dir && rc->umh) {
	if (vflag)
	    print("going for unionread\n");
	n = myunionread(rc, va, n);
	if (vflag)
	    print("done from unionread\n");
    } else {
	if (vflag)
	    print("not going for unionread\n");
...
enum {
    ...
    DIRREADLIM = 8048,		/* should handle the largest reasonable
				 * directory entry */
};
...
lsdir(char *name, Dir ** d)
{
    ...
    if (waserror()) {
	if (vflag)
	    print("readchandir failed\n");
	cclose(rock.cc);
	nexterror();
    }
    count = readchandir(rock.cc, d);
    if (vflag)
	print("done readchandir\n");
    cclose(rock.cc);
    if (vflag)
	print("freed cc\n");
    poperror();
...
fetchparentpathch(Chan * ch)
{
    ...
    strcpy(ptr, buff);
    if (vflag)
	print("filepath for qid[%llux] is [%s][%s]\n", ch->qid.path, buff,
	      ptr);
    return ptr;
}
